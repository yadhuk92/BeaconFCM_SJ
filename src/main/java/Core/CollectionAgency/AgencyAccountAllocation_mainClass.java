...
public class AgencyAccountAllocation_mainClass extends Base_Class{
	...
	CoreCollectionAgencyAgencyAccountAllocationRepo Page_Repository=new CoreCollectionAgencyAgencyAccountAllocationRepo();
   WebDriver driver;
...
public void executeHOUserCreation() {
    try {
    	...
    	String filePath = "CoreAgentAccountAllocation.properties"; // Adjust path as needed
		 ...
		 System.out.println("HOUserPassword: " + props.getProperty("HOUserPassword"));*/
		  ...
		  List<Object> inputParams = Arrays.asList(HOUserID,"John Doe", "john.doe@example.com","9876543210");
		  ...
		  //String fileName = "CoreHOUserCredentials_CoreUserManagement_HO_User_Creation.properties";
	        ...
	        Map<String, String> updates = Map.of(
	            "HOUserPassword", Password,
	            "HOUserID", UserID
	        );

	        PropertiesFileUtil.updateProperties(filePath, updates);
...
public boolean clickCollectionAgencyMenu() throws Throwable {
	Common.fluentWait("Collection Agency menu to display", CoreCollectionAgencyAgencyAccountAllocationRepo.collectionAgencyMenu);
	click(CoreCollectionAgencyAgencyAccountAllocationRepo.collectionAgencyMenu);
	return true;
}
//Method to click Agency a/c allocation
public boolean clickAgencyAccountAllocationMenu() throws Throwable {
	Common.fluentWait(" Agency Account Allocation menu to display", CoreCollectionAgencyAgencyAccountAllocationRepo.agencyAccountAllocationMenu);
	click(CoreCollectionAgencyAgencyAccountAllocationRepo.agencyAccountAllocationMenu);
	return true;
}
// Check visibility of 'Types of Account' dropdown
public boolean isTypesOfAccountDropdownVisible() {
	WebElement TypesOfAccountDropdown=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.TypesOfAccountDropdownlabel);
...
public boolean ValuesOfTypesOfDropdown() {
	List<WebElement> AllValue=driver.findElements(CoreCollectionAgencyAgencyAccountAllocationRepo.TypesOfAccountDrpAllValue);
...
public boolean clickassetCategory() {
	Common.fluentWait("AssetCategory", CoreCollectionAgencyAgencyAccountAllocationRepo.AssetCategory);
	WebElement assetCategory=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.AssetCategory);
	assetCategory.click();
	return true;
...
public boolean SelectNPA() {
	Common.fluentWait("NPA Category", CoreCollectionAgencyAgencyAccountAllocationRepo.NPACategory);
	WebElement NPACategory=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.NPACategory);
	NPACategory.click();
	return true;
}
...
public boolean Iszone_codVisible() {
	Common.fluentWait("Zone drop down", CoreCollectionAgencyAgencyAccountAllocationRepo.zone_co);
	WebElement zone=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.zone_co);
	zone.isDisplayed();
	return true;
}
//Select Amravati branch
public boolean selectAmravati(String Amar) throws Throwable {
	Common.fluentWait("branchSearch", CoreCollectionAgencyAgencyAccountAllocationRepo.branchSearch);
	WebElement search=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.branchSearch);
...
public String DPDValue(String DPD) {
	Common.fluentWait("DPDValue", CoreCollectionAgencyAgencyAccountAllocationRepo.DPDValue);
	WebElement InputDPD=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.DPDValue);
	InputDPD.clear();
...
public boolean DownLoadExcelDisabled() {
	WebElement wb=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.DownloadInExcelButton);
	wb.isEnabled();
	return false;
}

public boolean CheckBoxDispalyed() {
	Common.fluentWait("Check box", CoreCollectionAgencyAgencyAccountAllocationRepo.CheckBox);
	WebElement wb=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.CheckBox);
	wb.isDisplayed();
	return true;
}

public boolean DownLoadExcelEnabled() {
	Common.fluentWait("DownloadInExcelButton", CoreCollectionAgencyAgencyAccountAllocationRepo.DownloadInExcelButton);
	WebElement wb=driver.findElement(CoreCollectionAgencyAgencyAccountAllocationRepo.DownloadInExcelButton);
	wb.isEnabled();
	return true;
}
...
public boolean isExcelFileNameValid(String Allocation_List ) {
    ...
    System.out.println("Download directory for testing purpose: " + downloadDir);
...
public boolean isAgencyAllocationPageDisplayed() {
	Common.fluentWait("PreviousPage", Page_Repository.PreviousPage);
